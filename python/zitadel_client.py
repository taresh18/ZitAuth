...
class ZitadelClient:
    ...
    def validate_token(
        ...
        logger.info("JWKS cache: %s", self.jwks_cache)
        ...
        key = find_key(self.jwks_cache, kid)
        if not key:
            self.jwks_cache = requests.get(self.zitadel_jwks_url, timeout=10).json()
            key = find_key(self.jwks_cache, kid)
            if not key:
        ...
        decode_kwargs = {
            ...
            'algorithms': [alg],
            # Disable audience and at_hash verification to allow validating
            # ID tokens without passing the paired access_token
